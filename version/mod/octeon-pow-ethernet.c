...
#ifdef MULTIPLE_POW
static struct net_device *octeon_pow_ptp_dev_1;
static struct net_device *octeon_pow_ptp_dev_2;
static struct net_device *octeon_pow_ptp_dev_3;
#endif
...
static int octeon_pow_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	for (send_group = 0; send_group < octeon_pow_num_groups; send_group++) {
		...
		cvmx_wqe_set_port(work, priv->rx_group);
...
#ifdef MULTIPLE_POW
    /*pow1  group:12*/
    octeon_pow_ptp_dev_1 = alloc_etherdev(sizeof(struct octeon_pow));
	if (octeon_pow_ptp_dev_1 == NULL) {
		pr_err(DEV_NAME " ERROR: Failed to allocate ethernet device\n");
		return -1;
	}

	octeon_pow_ptp_dev_1->netdev_ops = &octeon_pow_netdev_ops;
	strcpy(octeon_pow_ptp_dev_1->name, "pow1"); // pow1 <----> group 12

	/* Initialize the device private structure. */
	priv = netdev_priv(octeon_pow_ptp_dev_1);
	priv->rx_group = 12;                 // pow1 <----> group 12
	priv->tx_mask = 1ull << ptp_tx_group;
	priv->is_ptp = true;

	if (register_netdev(octeon_pow_ptp_dev_1) < 0) {
		pr_err(DEV_NAME " ERROR: Failed to register ethernet device\n");
		free_netdev(octeon_pow_ptp_dev_1);
		return -1;
	}
    /*pow2  group:11*/
    octeon_pow_ptp_dev_2 = alloc_etherdev(sizeof(struct octeon_pow));
	if (octeon_pow_ptp_dev_2 == NULL) {
		pr_err(DEV_NAME " ERROR: Failed to allocate ethernet device\n");
		return -1;
	}

	octeon_pow_ptp_dev_2->netdev_ops = &octeon_pow_netdev_ops;
	strcpy(octeon_pow_ptp_dev_2->name, "pow2"); // pow2 <----> group 11

	/* Initialize the device private structure. */
	priv = netdev_priv(octeon_pow_ptp_dev_2);
	priv->rx_group = 11;                 // pow2 <----> group 11
	priv->tx_mask = 1ull << ptp_tx_group;
	priv->is_ptp = true;

	if (register_netdev(octeon_pow_ptp_dev_2) < 0) {
		pr_err(DEV_NAME " ERROR: Failed to register ethernet device\n");
		free_netdev(octeon_pow_ptp_dev_2);
		return -1;
	}

    /*pow3  group:10*/
    octeon_pow_ptp_dev_3 = alloc_etherdev(sizeof(struct octeon_pow));
	if (octeon_pow_ptp_dev_3 == NULL) {
		pr_err(DEV_NAME " ERROR: Failed to allocate ethernet device\n");
		return -1;
	}

	octeon_pow_ptp_dev_3->netdev_ops = &octeon_pow_netdev_ops;
	strcpy(octeon_pow_ptp_dev_3->name, "pow3"); // pow3 <----> group 10

	/* Initialize the device private structure. */
	priv = netdev_priv(octeon_pow_ptp_dev_3);
	priv->rx_group = 10;                 // pow3 <----> group 10
	priv->tx_mask = 1ull << ptp_tx_group;
	priv->is_ptp = true;

	if (register_netdev(octeon_pow_ptp_dev_3) < 0) {
		pr_err(DEV_NAME " ERROR: Failed to register ethernet device\n");
		free_netdev(octeon_pow_ptp_dev_3);
		return -1;
	}
...
#ifdef MULTIPLE_POW
    if (octeon_pow_ptp_dev_1) {
		unregister_netdev(octeon_pow_ptp_dev_1);
		free_netdev(octeon_pow_ptp_dev_1);
	}

    if (octeon_pow_ptp_dev_2) {
		unregister_netdev(octeon_pow_ptp_dev_2);
		free_netdev(octeon_pow_ptp_dev_2);
	}

    if (octeon_pow_ptp_dev_3) {
		unregister_netdev(octeon_pow_ptp_dev_3);
		free_netdev(octeon_pow_ptp_dev_3);
	}
#endif
}
